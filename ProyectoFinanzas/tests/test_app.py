...
def test_create_valid_expense_movement():
    category = Categoria("Comida")
    movement = Movimiento(category, "Gasto", 100.0)

    assert movement.category.name == "Comida"
...
def test_create_valid_income_movement():
    category = Categoria("Salario")
    movement = Movimiento(category, "Ingreso", 5000.0)

    assert movement.category.name == "Salario"
