...
class ConnectionManager:
    ...
    def load_mock_data(self, mock_data_filename):
        mock_data_path = os.path.join(
            os.path.dirname(__file__), mock_data_filename)
        try:
            with open(mock_data_path, 'r', encoding='utf-8') as file:
                return json.load(file)
        except FileNotFoundError:
            print(
                f"Error: No se encontró el archivo en la ruta {mock_data_path}")
            return None
        except json.JSONDecodeError:
            print(
                f"Error: El archivo {mock_data_filename} no es un JSON válido.")
            return None
        ...
        if mock_users:
            ...
            for user in mock_users:
                self.cursor.execute(user_query, (
                    user.get("first_name"), user.get(
                        "email"), user.get("username"),
                    user.get("password"), user.get(
                        "birthdate"), user.get("bank_account_state")
                ))
        ...
        if mock_cars:
            ...
            for car in mock_cars:
                self.cursor.execute(car_query, (
                    car.get("brand"), car.get(
                        "model"), car.get("year"), "available"
                ))
            print(f"Inserted {len(mock_cars)} mock cars.")

        self.connection.commit()

    def check_and_initialize_db(self):
        try:
            # Intentamos contar los usuarios para ver si la base de datos ya existe y tiene datos
            self.cursor.execute("SELECT COUNT(*) FROM users;")
            count = self.cursor.fetchone()[0]

            if count > 0:
                print(
                    "La base de datos ya se inicializo anteriormente. No se insertarán datos.")
                return

        except Exception:
            # Si la tabla no existe, PostgreSQL lanza un error.
            # Hacemos rollback para limpiar la transacción fallida y continuar.
            self.connection.rollback()
            print("Database empty or missing. Running initial setup...")

        # Si llegamos aquí, es porque la DB no existía o estaba vacía
        self.setup_database()
